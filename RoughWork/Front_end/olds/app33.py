...
""")
    ...
    with tab3:
        ...
        if st.button("🎯 Predict", type="primary", use_container_width=False,
            ...
            st.caption(
                "The minimum change to a single parameter of A "
                "that would reverse the outcome."
            )
            flips = []
            for p in params:
                orig_val = float(patient_a_input[p])
                for delta in [1, -1, 2, -2, 3, -3, 5, -5, 10, -10]:
                    test_a = {k: float(v) for k, v in patient_a_input.items()}
                    test_a[p] = orig_val + delta
                    fv2 = {}
                    for pp in params:
                        a2  = test_a[pp]
                        b2  = float(patient_b_input[pp])
                        pn2 = pp.replace("_", " ").title()
                        fv2[f"{pn2}: A-B"]        = a2 - b2
                        fv2[f"{pn2}: A\u00b2-B\u00b2"] = a2**2 - b2**2
                        fv2[f"{pn2}: A\u00b3-B\u00b3"] = a2**3 - b2**3
                        fv2[f"{pn2}: weaker"]     = min(a2, b2)
                        fv2[f"{pn2}: stronger"]   = max(a2, b2)
                        fv2[f"{pn2}: combined"]   = a2 + b2
                        fv2[f"{pn2}: gap"]        = abs(a2 - b2)
                    F2 = pd.DataFrame([fv2]).reindex(columns=feat_names, fill_value=0)
                    if rulefit.predict(F2.values)[0] != pred:
                        direction = "increase" if delta > 0 else "decrease"
                        flips.append({
                            "param":    p.replace("_", " ").title(),
                            "delta":    delta,
                            "dir_word": direction,
                            "new_pred": "A preferred" if rulefit.predict(F2.values)[0] == 1 else "B preferred",
                        })
                        break

            if flips:
                for fl in flips:
                    fc = COL_A if fl["new_pred"] == "A preferred" else COL_B
                    ds = f"+{fl['delta']}" if fl["delta"] > 0 else str(fl["delta"])
                    st.markdown(
                        f"<div style='border-left:3px solid {fc};"
                        f"padding:6px 14px;margin:4px 0;"
                        f"background:#f9fafb;border-radius:4px;font-size:13px'>"
                        f"If <b>{fl['param']}</b> of A changes by <b>{ds}</b> → "
                ...
                st.caption(
                    "No single-parameter change within ±10 flips this prediction. "
                    "The model is very confident."
                )

            st.divider()

            # ── 📊 Parameter contribution waterfall ────────────────────────
            st.markdown("#### 📊 Parameter contributions")
            st.caption(
                "How much each parameter difference pushes the prediction "
                "toward A (green) or B (red)."
            )
            # rulefit.coef aligns with rulefit.rules_, not feat_names
            # Build contributions by evaluating each rule's weighted contribution
            param_contribs = {}
            for p in params:
                param_contribs[p.replace("_", " ").title()] = 0.0

            for rule, coef in zip(rulefit.rules_, rulefit.coef):
                c = float(coef)
                if abs(c) < 1e-6:
                    continue
                # Find which param this rule mentions
                rule_str = rule.rule
                for p in params:
                    pn = p.replace("_", " ").title()
                    if pn.lower() in rule_str.lower():
                        # Check if rule fires for this input
                        try:
                            eval_str = rule_str
                            for fname, fval in zip(feat_names, F_new.values[0]):
                                eval_str = eval_str.replace(
                                    fname, str(round(float(fval), 4))
                                )
                            if eval(eval_str):
                                param_contribs[pn] = param_contribs.get(pn, 0.0) + c
                        except Exception:
                            pass
                        break
            param_contribs = {}

            pc_sorted  = sorted(param_contribs.items(), key=lambda x: x[1])
            names_pc   = [x[0] for x in pc_sorted]
            vals_pc    = [x[1] for x in pc_sorted]
            colors_pc  = [A_WIN if v >= 0 else B_WIN for v in vals_pc]

            dark_rcparams()
            fig_wf, ax_wf = plt.subplots(
                figsize=(9, max(3, len(names_pc) * 0.55)), facecolor=BG
            )
            ax_wf.set_facecolor(BG2)
            ax_wf.axvline(0, color=BORDER, linewidth=1.5, zorder=2)
            for i, (nm, vl, cl) in enumerate(zip(names_pc, vals_pc, colors_pc)):
                ax_wf.barh(i, vl, height=0.5, color=cl, alpha=0.85,
                           zorder=3, linewidth=0)
                ha   = "left"  if vl >= 0 else "right"
                xpos = vl + (0.008 if vl >= 0 else -0.008)
                ax_wf.text(xpos, i, nm, va="center", ha=ha,
                           fontsize=9, color=TEXT, zorder=4)
            ax_wf.set_yticks([])
            ax_wf.set_xlabel(
                "← pushes toward B          Contribution          pushes toward A →",
                fontsize=8, color=TEXT_DIM, labelpad=6
            )
            ax_wf.spines["left"].set_visible(False)
            ax_wf.spines["bottom"].set_color(BORDER)
            plt.tight_layout(pad=1.0)
            ...
            st.caption(
                "Move one parameter of A and see how the win probability changes "
                "in real time."
            )
            ...
            # Compute probs across a sweep
            sweep_vals = np.arange(curr_val - 10, curr_val + 11, 1.0)
            sweep_probs_a = []
            for sv in sweep_vals:
                test_a2 = {k: float(v) for k, v in patient_a_input.items()}
                test_a2[sens_param] = sv
                fvs = {}
                for pp in params:
                    a3  = test_a2[pp]
                    b3  = float(patient_b_input[pp])
                    pn3 = pp.replace("_", " ").title()
                    fvs[f"{pn3}: A-B"]        = a3 - b3
                    fvs[f"{pn3}: A\u00b2-B\u00b2"] = a3**2 - b3**2
                    fvs[f"{pn3}: A\u00b3-B\u00b3"] = a3**3 - b3**3
                    fvs[f"{pn3}: weaker"]     = min(a3, b3)
                    fvs[f"{pn3}: stronger"]   = max(a3, b3)
                    fvs[f"{pn3}: combined"]   = a3 + b3
                    fvs[f"{pn3}: gap"]        = abs(a3 - b3)
                Fs  = pd.DataFrame([fvs]).reindex(columns=feat_names, fill_value=0)
                if hasattr(rulefit, "predict_proba"):
                    pr = rulefit.predict_proba(Fs.values)[0]
                    sweep_probs_a.append(pr[1])   # prob of class 1 = A preferred
                else:
                    sweep_probs_a.append(float(rulefit.predict(Fs.values)[0]))

            # Current value prob for highlighted line
            curr_idx_sweep = int(np.argmin(np.abs(sweep_vals - sens_range)))
            curr_prob_a    = sweep_probs_a[curr_idx_sweep]

            light_rcparams()
            fig_sens, ax_s = plt.subplots(figsize=(9, 3.5))
            ax_s.plot(sweep_vals, sweep_probs_a, color="#3b82f6",
                      linewidth=2.5, zorder=3)
            ax_s.axhline(0.5, color="#9ca3af", linewidth=1,
                         linestyle="--", alpha=0.7)
            ax_s.axvline(sens_range, color="#f59e0b", linewidth=2,
                         linestyle="--", label=f"Current: {sens_range:.0f}")
            ax_s.fill_between(sweep_vals, sweep_probs_a, 0.5,
                              where=[p >= 0.5 for p in sweep_probs_a],
                              alpha=0.15, color=A_WIN, label="A preferred region")
            ax_s.fill_between(sweep_vals, sweep_probs_a, 0.5,
                              where=[p < 0.5 for p in sweep_probs_a],
                              alpha=0.15, color=B_WIN, label="B preferred region")
            ax_s.annotate(
                f"P(A) = {curr_prob_a:.0%}",
                xy=(sens_range, curr_prob_a),
                xytext=(sens_range + 0.5, curr_prob_a + 0.05),
