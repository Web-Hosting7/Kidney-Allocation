...
""")
    ...
    with tab3:
        ...
        # ── Always-visible prediction (no button needed) ───────────────────────
        # Runs immediately when inputs change — no button click required
        F_new  = _make_F(patient_a_input, patient_b_input)
        pred   = rulefit.predict(F_new.values)[0]
        prob   = (rulefit.predict_proba(F_new.values)[0]
